...
// Max antal kvitton per månad för ett Pro-abonnemang (skydd mot kontodelning & bots)
...
export async function POST(req: NextRequest) {
  ...
  const freeKey = `b2d:free:${ipHash}`;
  const proKey = `b2d:pro:${ipHash}`;

  // Läs av session_id från header eller URL
  const sessionId = req.headers.get("x-session-id") || req.nextUrl.searchParams.get("session_id");

  try {
    let isPro = false;

    // 1. Kontrollera om session_id är aktiv i Vercel KV
    if (sessionId) {
    ...
    // 2. Backup: kontrollera om enhetens IP-adress sparats som Pro
    if (!isPro) {
      isPro = Boolean(await kv.get(proKey));
    }

    // Månadstagg för automatisk månatlig nollställning (t.ex. "2026-09")
    const currentMonth = new Date().toISOString().slice(0, 7);
    const proId = sessionId || ipHash;
    const proUsageKey = `b2d:usage:pro:${proId}:${currentMonth}`;

    // 3. FAIR USE CHECK FÖR PRO: Kontrollera att månadskvoten inte är nådd
    if (isPro) {
      const currentProUsage = (await kv.get<number>(proUsageKey)) || 0;
      if (currentProUsage >= PRO_FAIR_USE_LIMIT) {
        return NextResponse.json({
          error: `Monatliches Fair-Use-Limit erreicht (${PRO_FAIR_USE_LIMIT} Belege). Bitte kontaktieren Sie support@beleg2buchhaltung.de für ein individuelles Firmenkontingent.`,
          usage: { count: currentProUsage, limit: 99999, remaining: 0 }
        }, { status: 429 });
      }
    }

    // 4. Gratisanvändare: tillämpa gratiskvoten (3 filer)
    if (!isPro) {
    ...
    if (!isPro) {
      ...
      // Öka förbrukningen för Pro och spara nyckeln i 60 dagar
      const newProCount = (await kv.incrby(proUsageKey, allFiles.length)) as number;
      await kv.expire(proUsageKey, 60 * 60 * 24 * 60);

      newUsage = { 
        count: newProCount, 
        limit: 99999, 
        remaining: Math.max(0, PRO_FAIR_USE_LIMIT - newProCount) 
      };
    }
